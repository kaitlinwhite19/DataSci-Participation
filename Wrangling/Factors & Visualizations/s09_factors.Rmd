...
now, filtering 
```{r}
select_countries <- c("Ireland", "Bangladesh", "Haiti", "Colombia", "Italy")

select_gap <- gapminder %>% 
  filter(country %in% select_countries)

nlevels(select_gap$country)
```
...
```{r}
select_gap_dropped <- select_gap %>% 
  droplevels()

nlevels(select_gap_dropped)
```
...
```{r}
ggplot(select_gap, aes(x = year, y = lifeExp,
                  color = fct_reorder(country, year))) +
  geom_line() +
  labs(color = "Country")
```
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_level(continent, "Oceania"))) +
  coord_flip()+
  theme_bw() 
```
