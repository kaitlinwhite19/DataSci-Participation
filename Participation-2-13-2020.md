...
```
## Error in gapminder %>% group_by(FILL_THIS_IN) %>% FILL_THIS_IN(min_life = min(lifeExp)): could not find function "%>%"
```
...
```
## Error in psych::bfi %>% as_tibble() %>% select(A1:A5) %>% rowwise() %>% : could not find function "%>%"
```
